...
mod edge_bc_tests {
    ...
    fn legacy_edge_sets_paint_nothing() {
        legacy_edge_sets_paint_nothing_impl()
    }

    /// The SECOND-MERGE INVARIANT, end to end at the geometry level: a
    /// legacy wind-tunnel scene's full projection — the model
    /// rasterizer with tunnel bands, then the edge painter with the
    /// preset (exactly what the app's update loop runs) — is
    /// byte-identical to the pre-T2-C rasterizer output alone. The
    /// solver kernels are untouched by both units, so identical layers
    /// + identical settings ⇒ identical simulation output.
    #[test]
    fn legacy_wind_tunnel_projection_is_unchanged() {
        use crate::model::{ObjMaterial, Shape, SketchModel, SketchObject};
        let (w, h, margin) = (64usize, 32usize, 4usize);
        let mut model = SketchModel::default();
        let id = model.fresh_id();
        model.add(SketchObject {
            id,
            shape: Shape::Ellipse { c: [20.0, 12.0], r: [6.0, 6.0], angle: 0.0 },
            ...
            fan_phase: 0.0,
            fan_angle: 0.0,
            smoke_rgb: [0.3, 0.8, 1.0],
            locked: false,
            hidden: false,
            parent: None,
        });
        let region = GridRect::full((w - 2 * margin) as usize, (h - 2 * margin) as usize);
        // Pre-T2-C behavior: the rasterizer alone.
        let mut a = Geometry::new(w, h);
        model.rasterize_region(&mut a, region, margin, true);
        // Post-merge behavior: rasterizer + edge painter with the preset.
        let mut b = Geometry::new(w, h);
        model.rasterize_region(&mut b, region, margin, true);
        paint_edge_bcs(&mut b, EdgeBcs::WIND_TUNNEL, region, margin);
        assert_eq!(a.cell, b.cell);
        assert_eq!(a.fan, b.fan);
        assert_eq!(a.dye_src, b.dye_src);
    }

    fn legacy_edge_sets_paint_nothing_impl() {
